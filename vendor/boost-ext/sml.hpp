...
#endif
template <class T, class U> struct is_empty_base : T { U _; };
template <class T>
struct is_empty
...
template <class...> struct join { using type = type_list<>; };
...
struct pool_type_impl<T &, aux::enable_if_t<aux::is_constructible<T>::value &&
                                            aux::is_constructible<T, T>::value>>
    : pool_type_base {
...
template <class T> missing_ctor_parameter<T> try_get(...) { return {}; }
...
#if defined(_MSC_VER) && !defined(__clang__)
  return detail::get_type_name<T, 39>(
      __FUNCSIG__, make_index_sequence<sizeof(__FUNCSIG__) - 39 - 8>{});
...
#elif defined(__GNUC__)
  return detail::get_type_name<T, 68>(
      __PRETTY_FUNCTION__,
...
template <class T> constexpr auto wrap(T callback) {
...
public:
  ...
  template <class T> queue_event(T object) {
...
struct queue_handler : queue_event_call<TEvents>... {
  ...
  explicit queue_handler(TQueue &queue)
      : queue_event_call<TEvents>(queue_handler::push_impl<TQueue, TEvents>)...,
        queue_{&queue} {}
  template <class TEvent> void operator()(const TEvent &event) {
...
struct transitions_sub<sm<TSM>, T, Ts...> {
  ...
  template <class _, class TEvent, class SM, class TDeps, class TSubs>
  static bool execute_impl(const back::on_exit<_, TEvent> &event, SM &sm,
                           TDeps &deps, TSubs &subs,
                           typename SM::state_t &current_state) {
    sub_sm<sm_impl<TSM>>::get(&subs).process_event(event, deps, subs);
    transitions<T, Ts...>::execute(event, sm, deps, subs, current_state);
...
template <class TSM> struct transitions_sub<sm<TSM>> {
  ...
  template <class, class SM, class TDeps, class TSubs>
  static bool execute(const anonymous &, SM &, TDeps &, TSubs &,
                      typename SM::state_t &) {
    return false;
  }
};
...
struct jump_table {
  ...
  static bool dispatch(sm_impl &, State &, const TEvent &, TDeps &, TSubs &,
                       const aux::type_list<> &) {
    return false;
  }
  template <int, class TMappings, class sm_impl, class State, class TEvent,
  ...
  static bool dispatch(sm_impl &self, State &current_state, const TEvent &event,
    ...
    case N:
      return get_state_mapping_t<TState, TMappings,
                                 typename sm_impl::has_unexpected_events>::
          template execute<TEvent, sm_impl, TDeps, TSubs>(event, self, deps,
                                                          subs, current_state);
    default:
...
struct fold_expr {
  ...
  static bool dispatch_impl(sm_impl &self, State &current_state,
...
namespace back {
template <class TSM>
struct sm_impl : aux::conditional_t<aux::is_empty<typename TSM::sm>::value,
  ...
  bool process_event(const TEvent &event, TDeps &deps, TSubs &subs) {
    ...
    do {
      do {
        ...
        deps, subs, aux::type<process_queue_t<TEvent>>{}, events_t{}));
    return handled;
  }
...
#else
    return process_event_except_imp<
  ...
  template <class TEvent, class TDeps, class TSubs,
            __BOOST_SML_REQUIRES(
                !aux::is_base_of<get_generic_t<TEvent>, events_ids_t>::value)>
  bool process_internal_generic_event(const TEvent &, TDeps &, TSubs &,
                                      state_t &) {
    return false;
  }
  template <class TEvent, class TDeps, class TSubs,
            __BOOST_SML_REQUIRES(
                aux::is_base_of<get_generic_t<TEvent>, events_ids_t>::value)>
  bool process_internal_generic_event(const TEvent &event, TDeps &deps,
...
#if !BOOST_SML_DISABLE_EXCEPTIONS
  template <class TMappings, class TEvent, class TDeps, class TSubs>
  bool process_event_except_imp(const TEvent &event, TDeps &deps, TSubs &subs,
                                aux::false_type) {
  ...
  bool process_defer_events(TDeps &, TSubs &, const bool,
  ...
  template <class TDeps, class TSubs, class TDeferQueue, class... TEvents>
  bool process_queued_events(TDeps &deps, TSubs &subs,
                             const aux::type<TDeferQueue> &,
                             const aux::type_list<TEvents...> &) {
    ...
    while (!process_.empty()) {
      (this->*dispatch_table[process_.front().id])(deps, subs,
                                                   process_.front().data);
      process_.pop();
    }
    return wasnt_empty;
...
public:
  ...
  explicit sm(TDeps &&deps)
      : deps_{aux::init{}, aux::pool<TDeps>{deps}}, sub_sms_{aux::pool<TDeps>{
                                                        deps}} {
    aux::get<sm_impl<TSM>>(sub_sms_).start(deps_, sub_sms_);
  }
  template <class... TDeps,
...
decltype(auto) get_arg(const aux::type<back::defer<TEvents...>> &, const TEvent,
                       TSM &sm, TDeps &) {
  return back::defer<TEvents...>{sm.defer_};
}
template <class... TEvents, class TEvent, class TSM, class TDeps>
decltype(auto) get_arg(const aux::type<back::process<TEvents...>> &,
...
#else
template <class T, class = void> struct state_sm { using type = state<T>; };
template <class T>
